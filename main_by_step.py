...
def read_data():
    ...
    return test, sales_train, shops, items, item_cats
...
def clean():
    global sales_train
    sales_train = sales_train[(sales_train.item_price < 10000) & (sales_train.item_price > 0)]
...
def step6():
    ...
    first_shop_item_buy_block = df[df['date_block_num'] > 0].groupby(['shop_id', 'item_id'])[
        'date_block_num'].min().reset_index()
    ...
    df = pd.merge(df, first_shop_item_buy_block[['item_id', 'shop_id', 'first_date_block_num']],
                  on=['item_id', 'shop_id'],
                  how='left')
...
def step7():
    global df
    # Add sales_train lags for last 3 months
    df = lag_feature(df, [1, 2, 3], 'item_cnt_month')
    # Add avg shop/item price

    index_cols = ['shop_id', 'item_id', 'date_block_num']
    group = sales_train.groupby(index_cols)['item_price'].mean().reset_index().rename(
        columns={"item_price": "avg_shop_price"},
        errors="raise")
    df = pd.merge(df, group, on=index_cols, how='left')
    ...
    group = sales_train.groupby(index_cols)['item_price'].mean().reset_index().rename(
        columns={"item_price": "avg_item_price"}, errors="raise")
...
def step8():
    ...
    item_id_target_mean = df.groupby(['date_block_num', 'item_id'])['item_cnt_month'].mean().reset_index().rename(
        columns={"item_cnt_month": "item_target_enc"}, errors="raise")
    ...
    item_id_target_mean = df.groupby(['date_block_num', 'item_id', 'city_code'])[
...
def step9():
    ...
    item_id_target_mean = df.groupby(['date_block_num', 'item_id', 'shop_id'])[
        'item_cnt_month'].mean().reset_index().rename(columns={
...
def step10():
    global df
    # For new items add avg category sales_train for last 3 months
    item_id_target_mean = df[df['item_first_interaction'] == 1].groupby(['date_block_num', 'item_category_code'])[
        'item_cnt_month'].mean().reset_index().rename(columns={
...
def step11():
    global df
    # For new items add avg category sales_train in a separate store for last 3 months
    item_id_target_mean = \
        df[df['item_first_interaction'] == 1].groupby(['date_block_num', 'item_category_code', 'shop_id'])[
            'item_cnt_month'].mean().reset_index().rename(columns={
...
def read_data_and_format():
    ...
    return X_train, Y_train, X_valid, Y_valid, X_test, feature_name
...
def train():
    ...
    lgb_train = lgb.Dataset(X_train[feature_name], Y_train.to_frame())
...
if __name__ == '__main__':
    test, sales_train, shops, items, item_cats = read_data()
    df = []
    ana()
    clean()
    feature()
    save_data()
    X_train, Y_train, X_valid, Y_valid, X_test, feature_name = read_data_and_format()
    gbm = train()
    predict()
